...
export const useAttendance = () => {
    ...
    const fetchAttendances = async () => {
        try {
            ...
            totalAttendances.value = response.data.data.pagination.total
        } catch (err: any) {
    ...
    const createAttendance = async (data: Partial<AttendanceRecord>) => {
        try {
            ...
            error.value = getErrorMessageAxios(err) || 'Gagal menambahkan data absensi'
            throw err
        } finally {
            loading.value = false
        }
    ...
    const fetchSummary = async (month?: number, year?: number) => {
        try {
            ...
            error.value = getErrorMessageAxios(err) || 'Gagal memuat ringkasan absensi'
            throw err
        } finally {
            loading.value = false
        }
    ...
    const totalPages = computed(() => Math.ceil(totalAttendances.value / itemsPerPage.value))
    ...
    const exportExcel = async () => {
        try {
            ...
            error.value = getErrorMessageAxios(err) || 'Gagal mengekspor data absensi'
