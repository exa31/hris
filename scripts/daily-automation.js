...
async function processDate(targetDateStr, options) {
  ...
  if (!isWorkDay && !options.includeWeekends) {
    console.log(`  ⏩ Skipping ${targetDateStr} (${dayNames[dayOfWeek]}) - Off-work day. Use --include-weekends to force.`)
  ...
  if (holidayRes.rows.length > 0 && !options.force) {
    console.log(`  🎉 Skipping ${targetDateStr} - Holiday: ${holidayRes.rows[0].name}. Use --force to run anyway.`)
