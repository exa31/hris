...
export const createEmployeeSchema = z.object({
  nip: z
    .number({ required_error: "NIP wajib diisi" })
    .positive("NIP harus berupa angka positif"),
  name: z
  ...
  marital_status: maritalStatusEnum,
  gender: genderEnum,
  children_count: z.number().nonnegative().default(0),
  join_date: z
    .string({ required_error: "Tanggal bergabung wajib diisi" })
    .refine((date) => !isNaN(Date.parse(date)), {
      message: "Format tanggal bergabung tidak valid",
    }),
  position_id: z.number({ required_error: "Jabatan wajib dipilih" }),
  department_id: z.number({ required_error: "Departemen wajib dipilih" }),
  type: employmentTypeEnum,
  birth_place_id: z.number({ required_error: "Tempat lahir wajib dipilih" }),
  status: z.boolean().default(true),
  district_id: z.number({ required_error: "Kecamatan domisili wajib dipilih" }),
  full_address: z
