...
} from "~~/server/model/announcement.model";

export async function getAnnouncements(
  client: PoolClient,
  params: SearchAnnouncementInput,
) {
  ...
  if (params.search) {
    conditions.push(
      `(an.title ILIKE $${paramIndex} OR an.content ILIKE $${paramIndex})`,
    );
...
export async function updateAnnouncement(
  client: PoolClient,
  id: number,
  data: Partial<UpdateAnnouncementInput>,
) {
  const fields: string[] = [];
  const values: any[] = [];
  let paramIndex = 1;
