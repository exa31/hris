...
import type { CreateAttendanceInput, UpdateAttendanceInput, SearchAttendanceInput } from '~~/server/model/attendance.model'
...
export async function createAttendance(client: PoolClient, data: CreateAttendanceInput) {
    const result = await client.query(
        `INSERT INTO attendances (employee_id, date, clock_in, clock_out, status, notes)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
        [data.employee_id, data.date, data.clock_in || null, data.clock_out || null, data.status, data.notes || null]
    )
    return result.rows[0]
}

export async function updateAttendance(client: PoolClient, id: number, data: Partial<UpdateAttendanceInput>) {
    const fields: string[] = []
    const values: any[] = []
    let paramIndex = 1
