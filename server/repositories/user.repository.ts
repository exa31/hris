...
export async function getUsers(client: PoolClient, params: SearchUsersInput) {
  ...
  const sort = validSortColumns.includes(sortColumn as string)
    ? sortColumn
    : "created_at";
...
export async function createUser(
  client: PoolClient,
  data: CreateUserInput & { password_hash: string },
) {
  const query = `
...
export async function updateUser(
  client: PoolClient,
  id: number,
  data: Partial<UpdateUserInput> & { password_hash?: string },
) {
  const fields = [];
  const values = [];
  let idx = 1;
...
export async function isEmployeeAlreadyUser(
  client: PoolClient,
  employeeId: number,
...
export async function searchEmployeesWithoutAccount(
  client: PoolClient,
  search: string,
) {
  const query = `
